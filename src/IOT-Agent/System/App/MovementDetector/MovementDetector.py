            ...
            if len(classids) > 0:
                ...
                for box in boxes:
                    ...
                    if self.curr_state == self.RECORDING:
                        if self.verbose:
                            pass
                            #self.ctx['__obj']['__log'].setLog(f"[INFO] {self.src_name} : Person detected, recording")
                    # restore recording start time, so that it doesn't stop recording
                    recording_start_time = time.time()
                if len(classids) == 1:
                    self.ctx['__obj']['__log'].setLog(f"[INFO] {self.src_name} : {len(classids)} person detected, recording")
                else:
                    self.ctx['__obj']['__log'].setLog(f"[INFO] {self.src_name} : {len(classids)} people detected, recording")
            # if recording, and remaining recording time is less than 0, stop recording
            elif self.curr_state == self.RECORDING:
                if self.verbose:
