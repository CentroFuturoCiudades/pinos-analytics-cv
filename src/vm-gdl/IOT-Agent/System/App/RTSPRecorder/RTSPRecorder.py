...
class RTSPRecorder(Borg):
    ...
    def update_queue(self):
        ...
        process = (
            ...
            .output('pipe:', format='rawvideo', pix_fmt='bgr24', s=f'{width}x{height}', r=10)
            ...
            try:
                ...
                if not in_bytes:
                    if time.time() - last_frame_time > self.inactive_timeout:
                        self.ctx['__obj']['__log'].setLog(f"[WARNING] [{self.ctx_rtsp["camera"]}] No frames received for {self.inactive_timeout} seconds, stream may be inactive.")
                        self.stream_active = False
                        process = (
                            ffmpeg
                            .input(self.rtsp_url, rtsp_transport='tcp')
    ...
    def startRecording(self):
        ...
        self.ffmpeg_proc = subprocess.Popen([
            ...
            "-c", "copy",
            "-f", "segment",
            "-segment_time", "15",  # 1 minute chunks as safety (adjust as needed)
            "-segment_format", "mp4",
            "-reset_timestamps", "1",
            "-movflags", "+faststart",
            "-strftime", "1",
