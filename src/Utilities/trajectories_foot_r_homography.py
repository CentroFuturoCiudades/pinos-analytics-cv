...
if ONE_VIDEO_ONLY:
    ...
    # Open the video file
    video_path = f"IOT-Agent/records/{DAY}/{CAMERA}/{TIMESTAMP}.mp4"
    ...
    if len(track_history) > 0:
        ...
        #plot trajectory
        cam_img = cv2.imread("../imgs/birdseye.jpg")
        ...
        # Transform points - add this for all the videos in a folder
        for track_id, points in track_history.items():
            if points:
    ...
    video_folder = f"IOT-Agent/records/{DAY}/{CAMERA}/"
    ...
    for video_path in video_files:
        ...
        if len(track_history) > 0:
            ...
            track_history = cv2.perspectiveTransform(track_history, HOMOGRAPHY) # in birdseye view
            track_history = track_history.reshape(-1, 2)
